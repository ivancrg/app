...
class RandomForestCV():
    ...
    def run(self):
        ...
        print("max_leaf_nodes:", best_rf_classifier.get_params()
              ['max_leaf_nodes'])
        print("max_features:", best_rf_classifier.get_params()['max_features'])
        print("max_depth:", best_rf_classifier.get_params()['max_depth'])
        print("min_samples_split:", best_rf_classifier.get_params()
              ['min_samples_split'])
        print("min_samples_leaf:", best_rf_classifier.get_params()
              ['min_samples_leaf'])

        with open(self.folder + f'/rf_gs_best_params.txt', 'w') as self.file:
            for key, value in best_rf_classifier.get_params().items():
                self.file.write(f'{key}: {value}\n')

        k = 5
        cv_scores = cross_val_score(
            best_rf_classifier, X_train_valid, y_train_valid, cv=k, scoring='accuracy')
        print(
            f'Cross validation with best grid search hyperparameters: {np.mean(cv_scores)}')
        sc = {'test_score': cv_scores}
        dd.visualize_cv(k, sc, self.folder, 'rf_gs_')

        rf_classifier = best_rf_classifier

        y_test_pred = rf_classifier.predict(X_test)
        dd.visualize_cr_cm(y_test, y_test_pred, self.folder, 'rf_gs_')
        ...
        plt.savefig(self.folder + '/rf_gs_fimp.png')
